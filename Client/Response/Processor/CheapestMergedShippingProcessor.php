...
class CheapestMergedShippingProcessor implements ResponseProcessorInterface
{
    /**
    ...
    /**
     * CheapestMergedShippingProcessor constructor.
    ...
    {
        foreach ($response['shippingOptions']['cheapestMergedShippingOptions'] ?? [] as $responseRate) {
            ...
            $rates = $this->rateBuilder->build(
                ShippingMethodManager::CHEAPEST_MERGED_SHIPPING . '_' . $carriePrefix . '_' . $responseRate['id'],
                $responseRate,
                ''
            );
