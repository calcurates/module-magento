...
#isSourced=0
if [[ -f "/magento2-2.4.9.tar.gz" || "${isSourced}" -eq "0" ]]; then
    echo "Copying the Magento2 template to the working directory..."
    tar -zxf "/magento2-2.4.9.tar.gz"
    rm "/magento2-2.4.9.tar.gz"
    cp -r ./magento2-2.4.9/* .
    rm -rf ./magento2-2.4.9
    ...
    curl -L -o /php-cs-fixer.phar https://github.com/FriendsOfPHP/PHP-CS-Fixer/releases/download/v3.95.11/php-cs-fixer.phar
