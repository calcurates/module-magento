...
until mariadb --skip-ssl --user="$MYSQL_USER" --password="$MYSQL_PASSWORD" --host="mysql" --port="3306" -e "SELECT VERSION()"; do
...
#isSourced=0
if [[ -f "/magento2-2.4.8-p3.tar.gz" || "${isSourced}" -eq "0" ]]; then
    echo "Copying the Magento2 template to the working directory..."
    tar -zxf "/magento2-2.4.8-p3.tar.gz"
    rm "/magento2-2.4.8-p3.tar.gz"
    cp -r ./magento2-2.4.8-p3/* .
    rm -rf ./magento2-2.4.8-p3
    ...
    curl -L -o /php-cs-fixer.phar https://github.com/FriendsOfPHP/PHP-CS-Fixer/releases/download/v3.91.3/php-cs-fixer.phar
