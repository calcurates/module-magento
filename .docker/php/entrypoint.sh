...
#isSourced=0
if [[ -f "/magento2-2.4.8-p4.tar.gz" || "${isSourced}" -eq "0" ]]; then
    echo "Copying the Magento2 template to the working directory..."
    tar -zxf "/magento2-2.4.8-p4.tar.gz"
    rm "/magento2-2.4.8-p4.tar.gz"
    cp -r ./magento2-2.4.8-p4/* .
    rm -rf ./magento2-2.4.8-p4
    ...
    curl -L -o /php-cs-fixer.phar https://github.com/FriendsOfPHP/PHP-CS-Fixer/releases/download/v3.95.1/php-cs-fixer.phar
